...
Description: Post part of a series on creating a point of sales system
            ...
            - Blog 21 Create layout for Payment page

Previous code: main5.py available on github
""" 
...
class Page2(tk.Frame):      # Login Page 
    ...
    def login(self) -> None:
        ...
        if success:
            ...
            # Update the label text in the Page to show employee's name
            self.ctrl.frames['Page3'].barista_lbl.configure(text=f"Barista: {self.ctrl.frames['Page3'].barista_name}")
            self.ctrl.frames['Page4'].barista_lbl.configure(text=f"Barista: {self.ctrl.frames['Page3'].barista_name}")
...
class Page3(tk.Frame):      # Order Page 
    ...
    def create_header_frame(self, parent:tk.Frame, title:str, format:str) -> tk.Frame:
        ...
        self.barista_lbl = ttk.Label(frame, text=f'Barista: ', style=format)
...
class Page4(tk.Frame):      # Payment Page 
    # Pass in the parent and controller parameters to the class
    def __init__(self, parent, controller):
        # Pass the parent parameter to the inherited tk.Frame
        super().__init__(parent)

        # Define the self.controller attribute
        self.ctrl = controller

        # Define the self.DATABASE constant based on the database path in the App class
        self.DATABASE = self.ctrl.COFFEE_DB

        # Use the title style that was configured in the App class
        self.ctrl.style.configure('Title.TLabel', 
                             font=(self.ctrl.TITLE_FONT)) 
        
        # Get from database
        self.PMT_TYPE = self.get_payment_types()

       # Define variable
        self.barista = ''               # Barista name from Page 2
        self.coffee_options = []        # Coffee info from Page 2
        self.emp_id = None              # Employee id from Page 1
        self.order_info = []            # Info on individual order from Page 2

        # define Entry variables
        self.payment_type_str = tk.StringVar(value='4')  # Set default to Cash
        self.due_str = tk.StringVar(value='')
        self.tendered_str = tk.StringVar(value='0.00')   # Set as default 
        self.change_str = tk.StringVar(value='')
        
        # Layout the overall page with one column and two rows. 
        self.columnconfigure(0, weight = 1)
        self.rowconfigure(0, weight = 1)
        self.rowconfigure(1, weight = 3)
        self.rowconfigure(2, weight = 2)

        # Call header frame method, and pass in Payment and style
        self.create_header_frame(parent=self, 
                                title='Payment', 
                                format='Title.TLabel')
        
        # Create frame and contents for Order Payment section
        self.payment_frm = self.create_payment_frame(self)
        self.payment_frm.grid(column=0, row=1, sticky=tk.EW)

        # Create return button
        order_entry_btn = ttk.Button(
            self, text="Return to Order Entry", command=lambda: self.ctrl.show_frame('Page3')
        )
        order_entry_btn.grid(column=0, row=2, pady=(40, 0))        

    # Define a method to create a header frame with title and style parameters. 
    # It should be a child of the page class. 
    def create_header_frame(self, parent:tk.Frame, title:str, format:str) -> tk.Frame:
        """
        Description: Create header frame section 
        Param: container - Parent grid container
        ...
        title_lbl = ttk.Label(frame, text=title, style=format)
        title_lbl.grid(column=1, row=0, sticky=tk.W)
        self.barista_lbl = ttk.Label(frame, text=f'Barista: Maya', style=format)
        self.barista_lbl.grid(column=2, row= 0, columnspan=2, sticky=tk.E)

        # Use the grid method to locate the header frame in the first column and row. 
        frame.grid(column=0, row=0, pady=(5, 0), sticky=tk.NW)
        return frame
   
    def get_payment_types(self) -> list[tuple]:
        """
        Description: Gets the payment types from Payments table 
                    in coffee db supplied in login page
        Param: None
        Returns      The payment info
        """
        coffee_sql = 'SELECT * FROM Payments'
        conn = dbu.create_connection(self.DATABASE)
        results = dbu.get_all_records(conn, coffee_sql, self.DATABASE)
        return results

    def create_payment_frame(self, parent: ttk.Frame) -> ttk.Frame:
        '''
        Description: Create frame for order payment
        Param: parent - Parent grid container
        Return: frame - order payment frame with content
        '''
        frame = ttk.LabelFrame(
            parent, text='  Payment  ')
        frame.grid(padx=(10, 10), pady=(15, 0))
        frame.config(borderwidth=1, relief=tk.SOLID)

        # Layout grid for order payment frame
        frame.columnconfigure(0, weight=1)
        frame.columnconfigure(1, weight=1)
        frame.rowconfigure(0, weight=1)

        # Create order payment Frame
        amounts_frm = ttk.Frame(frame)
        amounts_frm.grid(column=0, row=0, pady=(5, 0), sticky=tk.EW)

        # Layout grid for order frame
        amounts_frm.columnconfigure(0, weight=1)
        amounts_frm.columnconfigure(1, weight=3)
        amounts_frm.rowconfigure(0, weight=1)
        amounts_frm.rowconfigure(1, weight=1)
        amounts_frm.rowconfigure(2, weight=1)
        amounts_frm.rowconfigure(3, weight=1)
        amounts_frm.rowconfigure(4, weight=1)

        # Create payment frame
        ttk.Label(amounts_frm,
                  text='Due').grid(column=0, row=0, padx=35, sticky=tk.W)

        self.due_amt_ent = tk.Entry(
            amounts_frm, textvariable=self.due_str,
            justify=tk.RIGHT, width=10,
            borderwidth=1, state=tk.DISABLED, 
            disabledbackground='white', disabledforeground='Black')
        self.due_amt_ent.grid(column=1, row=0, pady=(0, 1))

        # Display amount tendered
        ttk.Label(amounts_frm,
                  text='Tendered').grid(column=0, row=1, padx=35, sticky=tk.W)

        self.tendered_ent = tk.Entry(amounts_frm,
                                      textvariable=self.tendered_str,
                                      justify=tk.RIGHT, width=10
                                      )
        self.tendered_ent.grid(column=1, row=1, 
                                padx=(5, 5), pady=(5, 5), sticky=tk.W)
        self.tendered_ent.selection_range(0, tk.END)

        # Create return display field
        ttk.Label(amounts_frm,
                  text='Return').grid(column=0, row=2, padx=35, sticky=tk.W)

        self.change_amt_ent = tk.Entry(
            amounts_frm, textvariable=self.change_str,
            justify=tk.RIGHT, width=10,
            borderwidth=1, state=tk.DISABLED, 
            disabledbackground='white', disabledforeground='Black')
        self.change_amt_ent.grid(column=1, row=2, pady=(0, 1))

        # Create Payment and Receipt buttons
        ttk.Button(amounts_frm, text='Payment').grid(column=0, row=3, 
                    padx=(40, 10), pady=(40, 10), sticky=tk.E)
        self.receipt_btn = ttk.Button(amounts_frm, text='Receipt')
        self.receipt_btn.grid(column=0, row=4, padx=(40, 10), 
                              pady=(5, 10), sticky=tk.E)
        self.receipt_btn.config(state=tk.DISABLED)
        # End of amount frame

        # Create payment type Frame
        payment_type_frm = ttk.Frame(frame)
        payment_type_frm.grid(column=1, row=0, pady=(5, 30), sticky=tk.EW)
  

        # Layout grid for payment type frame
        payment_type_frm.columnconfigure(0, weight=1)
        payment_type_frm.rowconfigure(0, weight=1)
        payment_type_frm.rowconfigure(1, weight=1)
        payment_type_frm.rowconfigure(2, weight=1)
        payment_type_frm.rowconfigure(3, weight=1)
        payment_type_frm.rowconfigure(4, weight=1)
        payment_type_frm.rowconfigure(5, weight=1)

        # Create Payment options
        ttk.Label(payment_type_frm, text='Payment Selections').grid(
            column=0, row=0, padx=(40, 0), sticky=tk.W)

        ttk.Radiobutton(payment_type_frm, text=self.PMT_TYPE[0][1], value=self.PMT_TYPE[0][0],
                        variable=self.payment_type_str).grid(column=0, row=1, padx=(55, 0), sticky=tk.W)
        ttk.Radiobutton(payment_type_frm, text=self.PMT_TYPE[1][1], value=self.PMT_TYPE[1][0],
                        variable=self.payment_type_str).grid(column=0, row=2, padx=(55, 0), sticky=tk.W)
        ttk.Radiobutton(payment_type_frm, text=self.PMT_TYPE[2][1], value=self.PMT_TYPE[2][0],
                        variable=self.payment_type_str).grid(column=0, row=3, padx=(55, 0), sticky=tk.W)
        ttk.Radiobutton(payment_type_frm, text=self.PMT_TYPE[3][1], value=self.PMT_TYPE[3][0],
                        variable=self.payment_type_str).grid(column=0, row=4, padx=(55, 10), sticky=tk.W)
        ttk.Radiobutton(payment_type_frm, text=self.PMT_TYPE[4][1], value=self.PMT_TYPE[4][0],
                        variable=self.payment_type_str).grid(column=0, row=5, padx=(55, 0), pady=(0, 15), sticky=tk.W)

        # bind a virtual event for focus
        # self.bind("<<Raised>>", self.on_raised)
        
        return frame


class Page5(tk.Frame):      # Daily Sales Totals Page 
    # Pass in the parent and controller parameters to the class
    def __init__(self, parent, controller):
        # Pass the parent parameter to the inherited tk.Frame
        super().__init__(parent)
        ...
        # Layout the overall page with one column and two rows. Set weight to 1.
        self.columnconfigure(0, weight = 1)
...
class Page6(tk.Frame):      # View Orders Page 
    # Pass in the parent and controller parameters to the class
    def __init__(self, parent, controller):
        # Pass the parent parameter to the inherited tk.Frame
        super().__init__(parent)
...
class Page7(tk.Frame):      # Filtered Orders Page 
    # Pass in the parent and controller parameters to the class
    def __init__(self, parent, controller):
        # Pass the parent parameter to the inherited tk.Frame
        super().__init__(parent)
...
class App(tk.Tk):  
    def __init__(self):
        ...
        # Add the Page4 class to the self.PAGES constant in the App class
    ...
    def create_menu_bar(self, main_win: tk.Tk) -> None:
        ...
        if self.logged_in:
            ...
            # Add the Payments option to the Page menu items section if the employee is logged in
            page_menu.add_command(
                label='Payment', command=lambda: self.show_frame('Page4'))
        else:
            page_menu.add_command(
